...
ZeroNet has builtin database, _SQLite_. Of course, you can create your own DB using files, but this is not serious. Let's begin with creating our database.

## 数据源

In ZeroNet, only SQL _SELECT_ is allowed. Database is created from JSON files. Let's create one of these files right now.
...
-   _SQLite_ database is only one file, and `db_file` is its path.
-   `version` is database, not _SQLite_ version. We'll talk about it later.
...
`cols` is column array. First entry of each column is column name (SQL keywords, such as `GROUP` and `UPDATE` are not allowed). Second entry of each column is _SQLite_ column type - `integer`, `float` or `text`.
